...
import composition.audit2.{AuditServiceReal, AuditMicroServiceCallFails, AuditMicroServiceCallNotOk}
...
final class AuditServiceImplSpec extends UnitSpec with MockitoSugar {
  ...
  private def auditServiceCallFails = testInjector(new TestConfig(), new AuditMicroServiceCallFails, new AuditServiceReal).getInstance(classOf[webserviceclients.audit2.AuditService])

  private def auditServiceCallNotOk = testInjector(new TestConfig(), new AuditMicroServiceCallNotOk, new AuditServiceReal).getInstance(classOf[webserviceclients.audit2.AuditService])
}