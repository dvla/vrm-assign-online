...
import webserviceclients.fakes.VrmAssignEligibilityWebServiceConstants.vrmAssignEligibilityResponseNinetyDayRuleError
...
final class VrmAssignEligibilityCallNinetyDayRuleError extends ScalaModule with MockitoSugar {

  val stub = {
    val webService = mock[VrmAssignEligibilityWebService]
    when(webService.invoke(any[VrmAssignEligibilityRequest], any[TrackingId])).
      thenReturn(Future.successful(createResponse(vrmAssignEligibilityResponseNinetyDayRuleError)))
    webService
  }
