...
class LeaveFeedbackUnitSpec extends UnitSpec {
  ...
  "present" should {
    "contain code in the page source to open the survey " +
      "in a new tab when a survey url is configured" in new TestWithApplication {
    ...
    "not contain code in the page source to open the survey " +
      "in a new tab when a survey url is configured" in new TestWithApplication {
