...
libraryDependencies ++= Seq(
  ...
  // VMPR
  "dvla" %% "vehicles-presentation-common" % "2.58" withSources() withJavadoc() exclude("junit", "junit-dep"),
  "dvla" %% "vehicles-presentation-common" % "2.58" % "test" classifier "tests" withSources() withJavadoc() exclude("junit", "junit-dep")
)
...
lazy val paymentSolveProject = paymentSolve("0.31").disablePlugins(PlayScala, SbtWeb)
lazy val vehicleAndKeeperLookupProject = vehicleAndKeeperLookup("0.26").disablePlugins(PlayScala, SbtWeb)
lazy val vrmAssignEligibilityProject = vrmAssignEligibility("0.23").disablePlugins(PlayScala, SbtWeb)
lazy val vrmAssignFulfilProject = vrmAssignFulfil("0.24").disablePlugins(PlayScala, SbtWeb)
